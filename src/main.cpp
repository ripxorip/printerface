...
void loop_wrap()
{
  ...
  {
    colorWipe(strip.Color(255, 230, 230), 10);
  }
...
const double fs = 1.00/((double)looptime*1e-3);
const double fc = 5.00;
const double fn = 2.00 * (fc/fs);

auto filter = butter<6>(fn);

const uint32_t com_time = 1000;
...
void loop()
{
  ...
  printerface_action_t action = com.rx();
  ...
  while (Serial.available() > 0)
  {
    ...
    {
      create_hue_palette(30000, 40000);
    }
